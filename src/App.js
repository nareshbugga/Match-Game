...
import ThumbnailItems from './Components/ThumbnailItems'
import WinORLossCard from './Components/WinORLossCard'
import './App.css'
...
class App extends Component {
  state = {
    ListId: tabsList[0].tabId,
    imageStatus: imagesList[0],
    score: 0,
    time: 60,
    status: true,
  }
  ...
  onMatch = thumbnailUrl => {
    ...
    if (result) {
      ...
      this.setState({imageStatus: imagesList[index]})
    } else {
      this.setState({status: false})
      clearInterval(this.timeId)
    }
  ...
  render() {
    const {ListId, imageStatus, score, time, status} = this.state

    console.log(time)
    const FilterList = imagesList.filter(
      eachList => eachList.category === ListId,
    )
        ...
        <div>
          {status ? (
            <div className="sub-container">
              <div>
                <img src={imageStatus.imageUrl} alt="match" className="Image" />
              </div>
              <ul className="tab-items">
                {tabsList.map(eachTab => (
                  <TabItems
                    ...
                    condition={ListId === eachTab.tabId}
                  />
                ))}
              </ul>
              <div>
            ...
            <div className="sub-container">
              <WinORLossCard score={score} onPlayAgain={this.onPlayAgain} />
            </div>
          )}
        </div>
      </div>
